...
float[][] yawValues = new float[1][maxSamples];
...
void drawArtificialHorizon(int x, int y, float[][] pyr)
{
  ...
  pgArtificialHorizonRing.rotate(radians(yawValues[0][actualSample-1]));
...
void draw() 
{
  ...
  drawChart("Pitch [deg]", compareSeries, pitchValues, 10, 10, 250, true, true, -90, 90, 30);
  drawChart("Roll [deg]", compareSeries, rollValues, 10, 330, 250, true, true, -90, 90, 30);
  drawChart("Yaw [deg]", compareSeries, yawValues, 10, 650, 250, true, true, -180, 180, 30);

  drawArtificialHorizon(700, 80, pyrValues);
  drawArtificialHorizon(700, 500, pyrValuesFiltered);
 
  drawChart("Accelerometer [m/s]", xyzSeries, accValues, 1400, 10, 400, true, true, -25, 25, 5);
  drawChart("Gyroscope", xyzSeries, gyrValues, 1400, 480, 400, true, true, -25, 25, 5);
}
...
void serialEvent (Serial myPort)
{
  ...
  if (inString != null)
  {
    ...
    // Fill Yaw compare
    yawValues[0][actualSample] = (float(list[4]));

    // Fill ACC compare
    accValues[0][actualSample] = (float(list[5]));
    accValues[1][actualSample] = (float(list[6]));
    accValues[2][actualSample] = (float(list[7]));

    // Fill GYR compare
    gyrValues[0][actualSample] = (float(list[8]));
    gyrValues[1][actualSample] = (float(list[9]));
    gyrValues[2][actualSample] = (float(list[10]));
